...
###### get.ks.statistic
########################################
# Computes the Kolmogorov-Smirnov (KS) statistic (using estimators of choice: MLE or mean-sd) for a 
# given dataset (data) against a specified theoretical family distribution (dist).

# Output:
# - Returns the parameter based Kolmogorov-Smirnov statistic as a numeric value.

get.ks.statistic<- function(data, dist="norm", shape = NULL, shape1 = NULL, shape2 = NULL, df = NULL, method = NULL){
  ...
  }, error = function(e) stop("'dist' must be a valid function name")) # Made global so user can check the CDF
  fdistri <- pdist(data.sort, mu = mu.MLE, sigma = sigma.MLE, shape = shape, shape1 = shape1, shape2 = shape2, df = df)
  DPlus <- max(c(0,(1:n)/n-fdistri))
  DMinus <- max(c(0,fdistri-(0:(n-1))/n))
  return(max(DPlus,DMinus))
}


get.ks.statistic.add.distribution<- function(data, mu.mle, sigma.mle, pdist, shape = NULL, shape1 = NULL, shape2 = NULL, df = NULL, method = NULL){
  data <- sort(data[complete.cases(data)])
  n <- length(data)
...
# Performs the parameter based Kolmogorov-Smirnov test to assess whether 
...
ks.test.LoScF<-function(x,dist="norm", shape = NULL, shape1 = NULL, shape2 = NULL, df = NULL, alfa=0.05,simulate.distH0=TRUE, nr=10^4, add.distribution=FALSE, method = "MLE"){
  ...
  if(isFALSE(add.distribution)){
    ...
    if (isTRUE(simulate.distH0)){
      ks.statistic.dist <- replicate(nr,{
        iter.sample<-rdist(length(x),0,1, shape = shape, shape1 = shape1, shape2 = shape2, df = df)
        get.ks.statistic(iter.sample,dist=dist, shape = shape, shape1 = shape1, shape2 = shape2, df = df, method = method)
      })
      
    }
    else if (is.vector(simulate.distH0)) {
      ks.statistic.dist <- simulate.distH0
    } 
    else {
      stop("'simulate.distH0' must be TRUE or a numeric vector.")
    }
    
    ks.statistic<-get.ks.statistic(x,dist=dist, shape = shape, shape1 = shape1, shape2 = shape2, df = df, method = method)
  }
    ...
    if (isTRUE(simulate.distH0)){
      ks.statistic.dist <- replicate(nr,{
        ...
        get.ks.statistic.add.distribution(iter.sample, mu.mle, sigma.mle, pdist, shape = shape, shape1 = shape1, shape2 = shape2, df = df, method = method)
      })
    }
    else if (is.vector(simulate.distH0)) {
      ks.statistic.dist <- simulate.distH0
    ...
    ks.statistic<-get.ks.statistic.add.distribution(x, mu.mle, sigma.mle, pdist, shape = shape, shape1 = shape1, shape2 = shape2, df = df, method = method)
  }
  else  {
    stop("'add.distribution' must be a list containing the Location-Scale
          MLE parameter general functions depending on the data.")
  }
  PVAL<-mean(ks.statistic.dist>=ks.statistic)
  
  structure(
    list(statistic = setNames(ks.statistic, "D"), p.value = PVAL, 
         method = "Lilliefors (Kolmogorov-Smirnov) type based test using MLE or mean-sd estimators", data.name = DNAME, 
...
# Simulates the parameter based Kolmogorov-Smirnov test statistic distribution under H0 using estimated parameters of choice (MLE or mean-sd).

simulate.distH0.ks <- function(size = 100, mu = 0, sigma = 1, nr = 10^4, dist, shape = NULL, shape1 = NULL, shape2 = NULL, df = NULL, method = "MLE"){
...
###### potencia.ks
########################################
# Performs a power analysis for the parameter based KS test
potencia.ks <- function(n, str.distH0, str.dist.used,
    ...
    distH0 <- do.call(simulate.distH0.ks, args.H0)
    ...
    rechazo <- replicate(10^4, {
      ...
      pvalor <- ks.test.LoScF(muestra, str.distH0, simulate.distH0 = distH0, method = method)$p.value
      pvalor <= alpha
    })
